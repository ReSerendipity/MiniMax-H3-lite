...
def _seed_image(client, sid) -> None:
    ...
    r = client.post(
        "/api/upload",
        files={"file": ("seed.png", b"fake", "image/png")},
        data={"shot_id": sid},
    )
...
def test_upload_duration_too_long_rejected_and_no_orphan(client, new_project, new_shot, monkeypatch):
    ...
    r = client.post(
        "/api/upload",
        files={"file": ("long.wav", b"fake", "audio/wav")},
        data={"shot_id": sid},
    )
...
def test_upload_duration_ok_and_meta_recorded(client, new_project, new_shot, monkeypatch):
    ...
    r = client.post(
        "/api/upload",
        files={"file": ("ok.wav", b"fake", "audio/wav")},
        data={"shot_id": sid},
    )
    assert r.status_code == 200, r.text
...
def test_upload_duration_total_exceeded(client, new_project, new_shot, monkeypatch):
    ...
    r = client.post(
        "/api/upload",
        files={"file": ("extra.mp4", b"fake", "video/mp4")},
        data={"shot_id": sid},
    )
    assert r.status_code == 422, r.text
...
def test_upload_pair_audio_to_video(client, new_project, new_shot, mock_ffprobe):
    ...
    rbad = client.post(
        "/api/upload",
        files={"file": ("bad.wav", b"fake", "audio/wav")},
    ...
    rv2 = client.post(
        "/api/upload",
        files={"file": ("c2.mp4", b"fake", "video/mp4")},
        data={"shot_id": sid2},
    )
    vid2 = rv2.json()["id"]
    rbad2 = client.post(
