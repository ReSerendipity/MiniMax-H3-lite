...
- `backend/routers/comfy_engine.py`（生产代码）：`C:\Users\Doro\APP\ComfyUI-aki-v3\ComfyUI\input|output` 两处硬编码改为**环境变量 + 项目内默认值**：`MMH3_COMFY_INPUT_DIR` / `MMH3_COMFY_OUTPUT_DIR`，默认 `data/comfy_input`、`data/comfy_output`。
- `requirements-frozen-20260913.txt`：torch / torchaudio / torchvision 的 `file:///C:/Users/Doro/.../_dl/manual/*.whl` 改为**官方 PyTorch 索引 URL**（`https://download.pytorch.org/whl/cu130/...`，文件名与 sha256 不变）。
- `scripts/clean_launch.py`：兄弟项目 WinPython 探测从本机硬编码改为**同级目录探测 + `MMH3_SIBLING_ROOT` 覆盖**；系统 CUDA Python 移除本机 ComfyUI 路径，保留 `C:\Python312` 并支持 `MMH3_EXTRA_PYTHON` 追加。
- `scripts/cleanup_garbage.py`：`BASE` 改为**`MMH3_REPO_ROOT` 环境变量覆盖**（默认仍为本机路径，标注 DEV-ONLY）。
- `scripts/_boot_check.py` / `_check_venvs.py`：头部标注 **DEV-ONLY**，项目根路径统一由 `DEV_REPO_ROOT` 环境变量推导（默认 `C:\Users\Doro`）。
- `scripts/dump_h3_wf.py`：头部标注 **DEV-ONLY**，ComfyUI 安装根统一由 `COMFYUI_ROOT` 环境变量推导（默认本机 aki-v3）。

## 6. 关联文档

- `docs/SRE_RUNBOOK.md`、`docs/version-baseline.md`
