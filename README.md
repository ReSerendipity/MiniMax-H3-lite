...
> 当前状态：前端模板化（Jinja2 单端口）与后端 API 全部就绪，官方工作流能力对齐（G1–G7）已完成，后端单测与前端冒烟本地验证通过（CI 状态以仓库 Actions 为准）；**真实模型端到端推理已在本地跑通**（`assets/generated/` 已产出 `h3_t2va_*.mp4` / `MiniMax_H3_*.mp4`），可走三模式页生成闭环，详见「模型与推理说明」。
...
> 💡 本项目依赖安装在项目根 `.venv`（系统 Python 3.12.10 @ `C:\Python312` 创建）。手动开发/测试请先 `.venv\Scripts\activate` 再运行下方命令。
